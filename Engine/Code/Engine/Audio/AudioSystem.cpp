...
AudioSystem* AudioSystem::m_audioInstance = nullptr;
...
AudioSystem* AudioSystem::GetInstance()
{
	...
	{
		m_audioInstance = new AudioSystem();
	}
...
//void AudioSystem::SetSoundGroup(const std::string& path)
//{
//
//}
...
SoundGroupID AudioSystem::CreateOrGetSoundGroup(const std::string& path)
{
	std::map< std::string, SoundGroupID >::iterator found = m_registeredGroupIDs.find( path );
	if( found != m_registeredGroupIDs.end() )
	{
		return found->second;
	}
	else
	{
		...
		m_fmodSystem->createSoundGroup(path.c_str(), &newGroup);
		if( newGroup )
		{
			SoundGroupID newGroupID = m_registeredGroups.size();
			m_registeredGroupIDs[ path ] = newGroupID;
			m_registeredGroups.push_back( newGroup );
			return newGroupID;
		}
	}
...
void AudioSystem::PlaySoundGroup(SoundGroupID)
{
